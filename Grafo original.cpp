...
Grafo* Grafo::arvore_geradora_minima_prim(std::vector<char> ids_nos) {
    Grafo* agm = new Grafo({in_direcionado ? '1' : '0', '1', '0', static_cast<char>(ids_nos.size() + '0')});
    if (ids_nos.empty()) return agm;
    
    // Alterado para sintaxe C++11
    ...
    incluidos.insert(ids_nos[0]);
    
    // Adiciona arestas do primeiro nó
    No* noInicial = encontrarNo(ids_nos[0]);
    if (noInicial) {
        for (Aresta* a : noInicial->arestas) {
            if (std::find(ids_nos.begin(), ids_nos.end(), a->id_no_alvo) != ids_nos.end()) {
                fila.push(std::make_pair(a->peso, std::make_pair(noInicial->id, a->id_no_alvo)));
            }
        }
    }
    
    while (!fila.empty()) {
        // Sintaxe C++11 em vez de structured binding
        std::pair<int, std::pair<char, char>> elemento = fila.top();
        ...
        if (incluidos.find(v) == incluidos.end()) {
            ...
            if (noV) {
                for (Aresta* a : noV->arestas) {
                    if (std::find(ids_nos.begin(), ids_nos.end(), a->id_no_alvo) != ids_nos.end() &&
                        incluidos.find(a->id_no_alvo) == incluidos.end()) {
                        fila.push(std::make_pair(a->peso, std::make_pair(v, a->id_no_alvo)));
                    }
                }
            }
        }
...
Grafo* Grafo::arvore_geradora_minima_kruskal(std::vector<char> ids_nos) {
    ...
    for (char id : ids_nos) {
        ...
        for (Aresta* a : no->arestas) {
            if (std::find(ids_nos.begin(), ids_nos.end(), a->id_no_alvo) != ids_nos.end()) {
                arestas.push_back({id, a->id_no_alvo, a->peso});
            }
        }
    }
    ...
    // Inicializa estruturas para Union-Find
    for (char id : ids_nos) {
        parent[id] = id;
        rank[id] = 0;
    }
